...
if(isset($_POST['update'])){
    ...
    if(is_uploaded_file($_FILES['product_img1']['tmp_name']) || is_uploaded_file($_FILES['product_img2']['tmp_name']) || is_uploaded_file($_FILES['product_img3']['tmp_name'])){
        ...
        $update_product = "update products set p_cat_id='$product_cat',date=NOW(),product_title='$product_title',product_price='$product_price',product_keywords='$product_keywords',product_desc='$product_desc',product_sale='$product_sale',product_label='$product_label' where product_id='$p_id'";
        
        $run_product = mysqli_query($con,$update_product);
        
        if($run_product){
            
        echo "<script>alert('Sản phẩm của bạn đã được cập nhật thành công (không upload ảnh)')</script>"; 
